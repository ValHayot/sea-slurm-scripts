...
do
    ...
    for c in "${conds[@]}"
    do
        echo "sbatch --output ${datadir}/logs/%x-%j-%N.out feat_hcp.sh $c $i $np ${stamp} ${data}"
        sbatch --job-name "${c}-feat" --output ${datadir}/logs/%x-%j-%N.out feat_hcp.sh $c $i $np ${datadir} ${data}
    done
done
